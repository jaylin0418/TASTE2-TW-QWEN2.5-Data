#!/bin/bash
# Per-node worker launcher for IF data TTS (Mode A: flat input file).
# Called by srun from tts_if.job.
# All env vars exported by parent job script.
...
for LOCAL_W in $(seq 0 $((N_WORKERS_PER_NODE - 1))); do
    ...
    CUDA_VISIBLE_DEVICES=$GPU_ID $PYTHON $REPO/tts/tts_runner.py \
        ...
        >> "$LOG_DIR/tts_if_node${SLURM_NODEID}_w${LOCAL_W}.log" 2>&1 &
done
