#!/bin/bash
# Per-node worker for UA remaining (Mode A: flat remaining_user_agent.jsonl).
# All env vars exported by tts_ua_remaining.job.
...
for LOCAL_W in $(seq 0 $((N_WORKERS_PER_NODE - 1))); do
    ...
    CUDA_VISIBLE_DEVICES=$GPU_ID $PYTHON $REPO/tts/tts_runner.py \
        ...
        >> "$LOG_DIR/tts_ua_rem_node${SLURM_NODEID}_w${LOCAL_W}.log" 2>&1 &
done
